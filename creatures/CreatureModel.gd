...
func _enter_tree():
	body = CreatureBody.new(sprite_texture)
	add_child(body)
...
func move_to_tile(tile: OrcGameMapTile) -> void:
	x = tile.x
	y = tile.y
	var viewport_position = get_tree().current_scene.grid_to_viewport(x, y)
	body.move_to_coords(viewport_position)
		
func path_to_tile(to_x: int, to_y: int):
	current_state = CreatureState.MOVING
	var position_id = Global.pathfinder.xy_to_index(x, y)
